<template>
	<div id="search" class="page">
		...
		<div class="search">
			...
			<input type="search" placeholder="搜电影，搜影院" v-model="val" />
			<img class="del-input" style="display: none;" alt="" src="../../assets/searchClose.png" @click="cancel()">
		</div>
		<div class="result">
			<search-movie :searchMovieList="this.searchMovieList" :movieTotal="this.movieTotal" v-if="movieTotal"></search-movie>
			...
			val: function() {
				getCinemaAndMovie(this.cityID, this.val).then(data => {
					if(data.newCinemaData) {
						...
						this.cinemaTotal=null;
					}
					if(data.newMovieData) {
						this.searchMovieList = data.newMovieData;
						this.movieTotal=data.movieTotal;
					}else{
						this.searchMovieList=[];
						this.movieTotal=null;
					}
